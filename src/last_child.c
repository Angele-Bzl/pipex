...
/*   Updated: 2025/03/06 14:51:43 by abarzila         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	manage_cmd_last(int *pipe_fd, char **arg, char **env)
{
	...
	if (outfile == -1)
		close_all_exit(0, pipe_fd, arg[4], EXIT_FAILURE);
	if (ft_strlen(arg[3]) == 0)
		close_all_exit(0, pipe_fd, "empty", EXIT_FAILURE);
	cmd_and_flags = ft_split(arg[3], ' ');
	if (!cmd_and_flags)
		close_all_exit(0, pipe_fd, "malloc", 127);
	path_cmd = find_real_cmd(env, cmd_and_flags);
	if (!path_cmd)
	{
		free_all(NULL, NULL, cmd_and_flags, NULL);
		ft_printf_err("%s: command not found\n", arg[3]);
		close_all_exit(outfile, pipe_fd, "path", 127);
	}
	...
	close_all_exit(outfile, pipe_fd, "execve", 127);
}
