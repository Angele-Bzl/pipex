...
/*   Updated: 2025/03/06 10:07:06 by abarzila         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	manage_cmd_last(int *pipe_fd, char **arg, char **env)
{
	...
	if (!path_cmd)
	{
		...
		close_fd_and_pipe_and_exit(outfile, pipe_fd, "command", 127);
	}
	if (access(path_cmd, X_OK))
	{
		free_all(NULL, NULL, cmd_and_flags, path_cmd);
		close_fd_and_pipe_and_exit(outfile, pipe_fd, "access", 127);
	}
	if (dup2(pipe_fd[0], STDIN_FILENO) == -1 || dup2(outfile, STDOUT_FILENO) == -1)
	{
		free_all(NULL, NULL, cmd_and_flags, path_cmd);
		close_fd_and_pipe_and_exit(outfile, pipe_fd, "dup2", 127);
	}
...
// 	path_cmd = NULL;
// 	cmd_and_flags = NULL;
// 	path_result = path(arg[3], env, cmd_and_flags, path_cmd);
// 	if (path_result != 0)
// 		close_fd_and_pipe_and_exit(outfile, pipe_fd, "command", path_result);
